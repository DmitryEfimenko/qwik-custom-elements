...
function renderComponentWrapper(input: {
  ...
  if (ssrAvailable === true) {
    lines.push('', `import { ${ssrBridgeName} } from './runtime';`, '');
    lines.push(
    ...
    lines.push(`import { ${csrBridgeName} } from './runtime';`, '');
...
function renderComponentWrapperRelativePath(
  componentTag: string,
  ssrAvailable: boolean | undefined,
): string {
  return `${componentTag}.${ssrAvailable === true ? 'ts' : 'tsx'}`;
}
