...
function createPlannedWrites(
  ...
  const adapterOwnedWrapperTags = new Set(
    adapterPlannedWrites.flatMap((plannedWrite) => {
      const extension = path.extname(plannedWrite.path);

      if (extension !== '.ts' && extension !== '.tsx') {
        return [];
      }

      const componentTag = path.basename(plannedWrite.path, extension);

      return componentTags.includes(componentTag) ? [componentTag] : [];
    }),
  );
  const wrapperWrites = componentDefinitions
    .filter(
      (componentDefinition) =>
        !adapterOwnedWrapperTags.has(componentDefinition.tagName),
    )
    .map((componentDefinition) => ({
      path: path.join(outDirPath, `${componentDefinition.tagName}.ts`),
...
function renderComponentWrapper(input: {
  ...
  const { projectId, componentDefinition, ssrMarkup } = input;
